...
sleep 30
docker exec writ-chain-bunkerweb-vapi-bunkerweb-1 nginx -T \
  > packages/cli/test/verify/__fixtures__/bunkerweb/nginx-T.txt
docker exec writ-chain-bunkerweb-vapi-bw-scheduler-1 \
  sh -c 'cat /data/configs/modsec/*/writ.conf' \
  > packages/cli/test/verify/__fixtures__/bunkerweb/scheduler-writ.conf
```
...
docker logs writ-chain-coraza-spoa-vapi-haproxy-1 \
  > packages/cli/test/verify/__fixtures__/coraza-spoa/haproxy.log 2>&1
docker logs writ-chain-coraza-spoa-vapi-coraza-spoa-1 \
