...
describe('firewall generator', () => {
  ...
  it('emits ACCEPT rule (before DROPs) for each domain in a URL allowlist', () => {
    ...
    const acceptIdx = out.indexOf('@@X_SECURITY_RESOLVE:hooks.example.com@@');
    const partnerIdx = out.indexOf('@@X_SECURITY_RESOLVE:api.partner.io@@');
  ...
  it('records provenance with `# x-security:` markers on every rule', () => {
    const out = buildIptablesV4(spec([]));
    const lines = out.split('\n');
    // Every non-empty, non-header, non-`-A` line in body should be a comment.
    // Simpler invariant: every `-A OUTPUT` line has a matching `# x-security:`
  ...
  it('appends a fail-closed default-deny terminator', () => {
    const out = buildIptablesV4(spec([]));
    assert.ok(out.includes('x-security/default-deny'));
    ...
    const terminatorIdx = out.indexOf('x-security/default-deny');
  ...
  it('uses iptables-restore-compatible header (*filter + COMMIT)', () => {
    const out = buildIptablesV4(spec([]));
    assert.ok(out.startsWith('# Generated by XSecurity'));
  ...
  it('emits deploy-time DNS wrapper scripts as additional artifacts', () => {
    ...
    for (const p of [
      'firewall/scripts/x-security-resolve.sh',
      'firewall/scripts/x-security-refresh.sh',
      'firewall/scripts/x-security-refresh.service',
      'firewall/scripts/x-security-refresh.timer',
      'firewall/scripts/x-security.logrotate',
  ...
  it('resolver and refresh scripts begin with a POSIX shebang', () => {
    ...
    const resolve = out.find((a) => a.path.endsWith('x-security-resolve.sh'));
    const refresh = out.find((a) => a.path.endsWith('x-security-refresh.sh'));
  ...
  it('resolver script guards against the three wave-7 busybox/alpine regressions', () => {
    ...
    //    documentation token `@@X_SECURITY_RESOLVE:<fqdn>@@` is not
  ...
  it('systemd timer fires every 5 minutes', () => {
    ...
    const timer = out.find((a) => a.path.endsWith('x-security-refresh.timer'))!;
    assert.ok(timer.content.includes('OnUnitActiveSec=5min'));
    assert.ok(timer.content.includes('[Timer]'));
  });

  it('README is non-empty and documents X_SECURITY_APP_UID', () => {
    const out = firewallGenerator.generate(spec([])) as Array<{
      path: string;
      content: string;
    }>;
    ...
    assert.ok(
      readme.content.includes('X_SECURITY_APP_UID'),
      'README must document X_SECURITY_APP_UID requirement'
    );
  });
