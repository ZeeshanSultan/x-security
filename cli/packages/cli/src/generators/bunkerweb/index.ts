 ...
 *
 * What this generator owns: phase-1/phase-2 Writ rules in plain
...
const MODSEC_BLOCK_MARKER = '# Writ-generated authentication rules';
...
function detectMixedJwtAudienceIssuer(per: EndpointSettings[]): string[] {
  ...
  for (const { settings } of per) {
    if (settings.WRIT_AUTH_AUDIENCE !== undefined) audiences.add(String(settings.WRIT_AUTH_AUDIENCE));
    if (settings.WRIT_AUTH_ISSUER !== undefined) issuers.add(String(settings.WRIT_AUTH_ISSUER));
  }
  const errors: string[] = [];
...
function serializeModsecConf(
  ...
  // W21-D: SSRF blocks (tagged `writ-rule-ssrf`) must precede the auth
  ...
  const ssrfBlocks = modsecEntries.filter(([, v]) =>
    String(v).includes('writ-rule-ssrf')
  );
