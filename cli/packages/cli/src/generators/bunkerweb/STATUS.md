...
| `deprecated` | SecRule id:970500+ status:410, tag `x-security-deprecated-endpoint-block` |
...
|-------|--------|--------|-----------|
| `injectionGuard += "deserialization"` | `request.schema.injectionGuard` | full | phase:2 `@rx` preamble denylist (node-serialize/Java rO0/PHP `O:n:`/pickle) on `ARGS:json.<f>\|ARGS:<f>`, tag `x-security-ssec-injection` |
| `injectionGuard += "ai-prompt"` | `request.schema.injectionGuard` | full | phase:2 `@rx` prompt-injection heuristic denylist, tag **`x-security-ssec-prompt`** (SSEC-PROMPT, not SSEC-INJECTION) |
| `authentication.passwordPolicy` | `authentication.passwordPolicy` | full | phase:2 `!@rx` per-requirement (minLength/upper/digit/symbol) + blocklist `@rx` on the body password field, 422, tag `x-security-rule-password-policy` |
| `authentication.accountLockout` | `authentication.accountLockout` | full | stateful `initcol:global` + `setvar`/`expirevar` failed-login counter (phase:5 increment on 401/403/422), `@gt attempts` deny 429. init/deny phase tracks the identifier source (header→phase:1, body field→phase:2). tag `x-security-rule-account-lockout` |
| `response.forbidArrayRoot` | `response.forbidArrayRoot` | full | phase:4 `RESPONSE_BODY @rx ^[\s…]*\[` bare-array deny 500, tag `x-security-rule-forbid-array-root` |
| `request.idempotencyKey` | `request.idempotencyKey` | **partial** | phase:1 missing-header 400 + persistent-collection replay dedupe (`@gt 1` → 409). Stops cross-request replay, NOT concurrent in-flight races (no atomic check-and-set at the WAF — schema itself flags partial). tag `x-security-rule-idempotency-key` |
| `logging` | `logging` | **partial** | phase:5 `log,auditlog` opt-in for request/response events (tag `x-security-audit`); injection-block/authz-deny/rate-limit-trip already audit-logged by their deny rules. NOT enforced at libmodsec3: per-event sink routing, `http-collector` sinkRef, `piiRedaction` — surfaced as a commented operator note |
...
| Path | Format | Purpose |
|------|--------|---------|
| `configs/modsec/x-security.conf` | conf | Phase-1/phase-2 x-security ModSecurity rules (plain SecRule directives). |
...
the bottom of `x-security.conf` includes a commented summary of recommended
...
- `plugins/x-security/jwt-verify.lua` — BunkerWeb's libmodsec3 lacks Lua, so
  `SecRuleScript` cannot run. The Lua approach is abandoned.
- `plugins/x-security/plugin.json` — paired with the abandoned Lua plugin.
...
| `authentication.type=basic`     | `USE_AUTH_BASIC=yes` (compose env) + Basic-credentials SecRule in modsec/x-security.conf. |
| `authentication.type=mtls`      | `USE_CLIENT_SSL=yes` (compose env; cert pinning unsupported).      |
| `authentication.type=bearer-jwt\|oauth2` | **Header-presence SecRule chain only** in modsec/x-security.conf. **Signature validation is NOT performed** — libmodsec3 has no Lua. Use an OIDC sidecar (e.g. oauth2-proxy) or Kong+OIDC in front of BunkerWeb for real JWT verification. Structured warning emitted to stderr + DEPLOYMENT.md at generate time. |
| `authentication.type=api-key`   | Header-presence SecRule in modsec/x-security.conf. **Value is not verified against any allowlist** — operator must enforce key validity at an upstream layer or via BunkerWeb-external mechanism. |
...
x-security cannot generate real JWT signature validation for the BunkerWeb
...
- **Kong with the OIDC plugin** (`--kong-edition enterprise`) — use x-security's
  `--target kong` instead, which does verify signatures natively.
- **Coraza-SPOA** chain — use x-security's `--target coraza-spoa`; that profile
...
| `authorization.type=rbac` (multi-role) | **full** | W23: SecRule chain (id:970600+) on `X-Forwarded-Groups` with alternation rx across declared roles. Tag `x-security-rule-rbac-multi-role`. |
| `cacheable` | override-only | |
| `response.errorScrubbing.stripStackTraces` | **full** | W23: phase:4 SecRule (id:268xxx) denies on stack-frame patterns in RESPONSE_BODY. Tag `x-security-output-sanitization`. |
...
| `request.schema.pii` (and response.schema sensitive-named fields) | **full** | W23: phase:4 SecRule (id:428xxx) per field. Tag `x-security-data-exposure`. |
...
| `deprecated` | **full** | W23: phase:1 SecRule (id:970500+) returns 410 with tag `x-security-deprecated-endpoint-block` (attribution.py:35). `sunsetDate` surfaced in msg. |
...
- `CUSTOM_CONF_MODSEC_*`: dedupe by `# x-security-generated authentication rules (<type>)`
...
- `USE_*`: any `yes` wins.
- `X_SECURITY_AUTH_ISSUER` / `X_SECURITY_AUTH_AUDIENCE`: mixed values across
...
grep -c "id:990000" /tmp/out/configs/modsec/x-security.conf   # → 1 (dedupe holds)
grep -oE "id:99[0-9]{4}" /tmp/out/configs/modsec/x-security.conf | sort -u | wc -l
...
docker run --rm --user root \
  -v /tmp/out/configs/modsec/x-security.conf:/x-security.conf:ro \
  owasp/modsecurity-crs:nginx sh -c '
    mkdir -p /tmp/m && cp /x-security.conf /tmp/m/
    echo "SecRuleEngine On" > /tmp/m/main.conf
    echo "Include /tmp/m/x-security.conf" >> /tmp/m/main.conf
